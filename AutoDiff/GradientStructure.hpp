...
namespace atl {
    ...
    class GradientStructure {
        ...
        //        //        typedef SecondOrderTable<REAL_T> SecondOrderMixed;
        //
        //        typedef std::unordered_map<uint32_t, std::unordered_map<uint32_t, FirstOrder> > ThirdOrderMixed;

        atl::FirstOrderTable<REAL_T> first_order;
        atl::SecondOrderTable<REAL_T> second_order_mixed;
        atl::ThirdOrderTable<REAL_T> third_order_mixed;
        ...
        std::vector<uint32_t> so_indexes = {0, 0};
        std::vector<uint32_t> to_indexes = {0, 0};
    public:
        uint32_t max_id = std::numeric_limits<uint32_t>::min();
        uint32_t min_id = std::numeric_limits<uint32_t>::max();
        ...
        inline REAL_T& Reference(uint32_t i, uint32_t j) {
            ...
            so_indexes = {i, j};
            std::sort(so_indexes.begin(), so_indexes.end());
            return second_order_mixed(so_indexes[0], so_indexes[1]);
            //            return second_order_mixed[so_indexes[0]][so_indexes[1]];
        }
        ...
        inline const REAL_T Value(uint32_t i, uint32_t j) {
            ...
            so_indexes = {i, j};
            std::sort(so_indexes.begin(), so_indexes.end());
            return second_order_mixed.get(so_indexes[0], so_indexes[1]);
            //            so_iterator it = this->second_order_mixed.find(so_indexes[0]);
            //
            //            if (it != this->second_order_mixed.end()) {
            //                fo_iterator itt = (*it).second.find(so_indexes[1]);
        ...
        inline const REAL_T Value_No_Sort(uint32_t i, uint32_t j) {
            return second_order_mixed.get(i, j);
        ...
        inline const REAL_T Value(uint32_t i, uint32_t j, uint32_t k) {
            std::vector<uint32_t> indexes = {i, j, k};
            std::sort(indexes.begin(), indexes.end());
            return third_order_mixed.get(indexes[0], indexes[1], indexes[2]);
...
#pragma unroll
                    ...
                    do {
                        ...
                        if ((j >= rows) && (k >= rows)) {
                            if (dk == REAL_T(0.0) && dj == REAL_T(0.0)) {
                                continue; //pushed variables didn't matter
                            }
                        }
                        ...
                        if (/*std::fabs(entry)*/entry != REAL_T(0.0) && entry == entry) {//h[j][k] needs to be updated
                            ...
                            //                            std::cout<<"Var["<<vj->id<<","<<vj->dependence_level<<"]\n";
                            //                            std::cout<<"Var["<<vk->id<<","<<vk->dependence_level<<"]\n";
                        ...
                        //                        if (i > 0) {
                        //                            if (gradient_stack[i].w->dependence_level > 0) {
                        //                                if (!pushed_js[j]) {
                        //                                    //this variable may be needed in the future, so push it to the next entry
                        //
                        //                                    gradient_stack[i - 1].PushVariable(vj);
                    ...
                    if (gradient_stack[i].w->dependence_level > 0) {//this was a compound assignment and its dependencies must be pushed
                        if (i > 0) {
                            //                            if(this->Value(gradient_stack[i].w->id,gradient_stack[i].w->id)!=static_cast<REAL_T>(0)){
                            gradient_stack[i - 1].PushVariables(gradient_stack[i].id_list);
                            //                            }
...
#pragma unroll
                    ...
                    do {
                        ...
                        if (j == 0) {
                            ...
                            if ((l >= rows) && (k >= rows)) {
                                if (hdk == REAL_T(0.0) && hdj == REAL_T(0.0)) {
                                    goto SKIP; //pushed variables didn't matter
                                }
                            }
...
#pragma unroll
        ...
        void PrepareDerivativeTables(int level = 2) {
            ...
            //                std::pair<uint32_t, uint32_t> p = this->gradient_stack[i].FindMinMax();
            ...
            this->first_order.Reset(this->min_id, this->max_id + 1);

            if (level == 2) {
                this->second_order_mixed.Reset(this->min_id, this->max_id + 1);
            }
            if (level == 3) {
                this->second_order_mixed.Reset(this->min_id, this->max_id + 1);
                this->third_order_mixed.Reset(this->min_id, this->max_id + 1);
            }
        }
