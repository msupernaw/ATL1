...
namespace atl {
    ...
    const Matrix<typename atl::PromoteType<typename LHS::RET_TYPE, typename RHS::RET_TYPE>::return_type > OuterProduct(
            const VectorExpression<typename LHS::RET_TYPE, LHS> &lhs, const VectorExpression<typename RHS::RET_TYPE, RHS> &rhs) {

        if (lhs.Size(0) != rhs.Size(0)) {
            std::cout << "Invalide Arguments to " << __func__ << ", lhs and rhs dimensions do not match.\n";
            exit(0);
        }

        size_t size = lhs.Size(0);

        Matrix<typename atl::PromoteType<typename LHS::RET_TYPE, typename RHS::RET_TYPE>::return_type > ret(size, size);
