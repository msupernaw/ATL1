...
namespace atl {
    ...
    public:
        ...
        void Check() {
            ...
            atl::Variable<T>::gradient_structure_g.derivative_trace_level = FIRST_ORDER;
//            this->Initialize();
            atl::Variable<T> f;
            //
            T hmin;
...
this->Objective_Function(f);
            ...
            //                    this->Objective_Function(f);
            //                    atl::Variable<T>::gradient_structure_g.HessianAndGradientAccumulate();
            //                    atl::Variable<T>::gradient_structure_g.Reset();
            ...
            for (int i = 0; i < this->check_parameters_m.size(); i++) {
                ...
                for (int j = 0; j < this->check_parameters_m.size(); j++) {
                    ...
                    exact_hessian[i][j] = atl::Variable<T>::gradient_structure_g.Value(this->check_parameters_m[i]->info->id, this->check_parameters_m[j]->info->id); //this->check_parameters_m[i]->info->hessian_row(this->check_parameters_m[j]->info->id);
#else
                    exact_hessian[i][j] = atl::Variable<T>::gradient_structure_g.Value(this->check_parameters_m[i]->info->id, this->check_parameters_m[j]->info->id); //this->check_parameters_m[i]->info->hessian_row(this->check_parameters_m[j]->info->id);this->check_parameters_m[i]->info->hessian_row[this->check_parameters_m[j]->info->id];
#endif
                }
            }
            std::cout << "done!\n";
...
//                    exit(0);
                ...
                for (int j = 0; j < this->check_parameters_m.size(); j++) {
                    for (int k = 0; k < this->check_parameters_m.size(); k++) {
                        if (i == 0) {
                            exact_hessian2[j][k] = atl::Variable<T>::gradient_structure_g.Value(this->check_parameters_m[j]->info->id, this->check_parameters_m[k]->info->id);
                        ...
                        third_mixed_exact[i][j][k] = atl::Variable<T>::gradient_structure_g.Value(this->check_parameters_m[i]->info->id, this->check_parameters_m[j]->info->id, this->check_parameters_m[k]->info->id);
                    }
                }
            }
            ...
            for (int i = 0; i < this->check_parameters_m.size(); i++) {
                ...
                for (int j = 0; j < check_parameters_m.size(); j++) {
                    for (int k = 0; k < check_parameters_m.size(); k++) {
                        h1[j][k] = atl::Variable<T>::gradient_structure_g.Value(check_parameters_m[j]->info->id, check_parameters_m[k]->info->id); //check_parameters_m[j]->info->hessian_row[check_parameters_m[k]->info->id];
                    }
                }
                ...
                for (int j = 0; j < check_parameters_m.size(); j++) {
                    for (int k = 0; k < check_parameters_m.size(); k++) {
                        h2[j][k] = atl::Variable<T>::gradient_structure_g.Value(check_parameters_m[j]->info->id, check_parameters_m[k]->info->id); //check_parameters_m[j]->info->hessian_row[check_parameters_m[k]->info->id];
                    }
                }
                atl::Variable<T>::gradient_structure_g.Reset();
#else
